...
export default function RAGChatInterface() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const errorMessage = {
        type: 'error',
        content: `Error: ${error.message}. Make sure the backend is running at ${apiUrl}`,
        timestamp: new Date()
      };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      <div className="container mx-auto p-6 max-w-6xl">
              ...
              <div className="h-full flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-slate-500 text-sm mt-2">
                    Try: "What is machine learning?" or "Explain neural networks"
                  </p>
                </div>
              </div>
            )}
                        ...
                        {message.retrievedDocs.map((doc, i) => (
                          <div key={i} className="text-xs bg-black/20 rounded p-2">
                            <div className="flex items-center justify-between mb-1">
                              <span className="font-mono text-purple-300">{doc.id}</span>
          ...
          <div className="border-t border-white/20 p-4 bg-black/20">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/10 border border-white/20 rounded-lg px-4 py-3 focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-400/20 placeholder-slate-500"
                disabled={loading}
              />
              <button
                onClick={handleSubmit}
                disabled={loading || !input.trim()}
                className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg px-6 py-3 font-semibold transition-all transform hover:scale-105 flex items-center gap-2"
              >
                {loading ? (
            ...
            <div className="mt-3 flex items-center gap-2 text-xs text-slate-400">
              ...
              <input
                ...
                className="bg-white/5 border border-white/10 rounded px-2 py-1 text-slate-300 font-mono text-xs"
        ...
        <div className="mt-6 text-center text-slate-400 text-sm">
          <p>Custom Vector DB with Flat Index | BGE-micro Embeddings | GPT-2 Generation</p>
        </div>
      </div>
    </div>
  );
